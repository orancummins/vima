...
def register(bp):
    @bp.route("/benefits_eligibility/benefits/searches", methods=["POST"])
    def search_benefits():
        store.lazy_load(API)
        benefits = store.list(API, "benefits")
        return jsonify({"data": benefits})

    @bp.route("/benefits_eligibility/products/searches", methods=["POST"])
    def search_products():
        store.lazy_load(API)
        products = store.list(API, "products")
        return jsonify({"data": products})

    @bp.route("/benefits_eligibility/widgets/access-tokens", methods=["GET"])
    def access_token():
        store.lazy_load(API)
