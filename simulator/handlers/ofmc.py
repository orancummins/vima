...
def register(bp):
    @bp.route("/ofmc/loyalty/offers/eop/categories/category-searches", methods=["POST"])
    def search_categories():
        store.lazy_load(API)
        categories = store.list(API, "categories")
        return jsonify({"categories": categories, "totalCount": len(categories)})

    @bp.route("/ofmc/loyalty/offers/eop/sources", methods=["GET"])
    def list_sources():
        store.lazy_load(API)
        sources = store.list(API, "sources")
        return jsonify({"sources": sources, "totalCount": len(sources)})

    @bp.route("/ofmc/loyalty/offers/eop/sources/<source_uuid>", methods=["GET"])
    ...
    @bp.route("/ofmc/loyalty/offers/eop/merchants", methods=["POST"])
    def create_merchant():
        store.lazy_load(API)
    ...
    @bp.route("/ofmc/loyalty/offers/eop/merchants/<merchant_id>", methods=["GET"])
    ...
    @bp.route("/ofmc/loyalty/offers/eop/merchants/<merchant_id>", methods=["PUT"])
